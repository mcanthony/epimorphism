...
_EPI_ float4 rgb_id(float4 v, float2 z_z, float* par, float time){
  // identity
  // FULL, LIVE, DEV
  return v;
}

_EPI_ float4 rotate_hsv(float4 v, float2 z_z, float* par, float time){
  // hsv rotation
  // FULL, LIVE, DEV
  ...
  if(_COLOR_TH_EFF != 0.0f && (z_z.y != 0.0f || z_z.x != 0.0f)){
    a = atan2(z_z.y, z_z.x) * floorf(8.0f * _COLOR_TH_EFF) / (2.0f * PI);
  }
...
_EPI_ float4 rotate_hsls(float4 v, float2 z_z, float* par, float time){
  // complex hsls rotation
  // FULL, LIVE, DEV

  v = RGBtoHSLs(v);

  //float l = recover(2.0 * log(5.0 * len(z_z) / (len(z_z) + 1.0)));
  ...
  float3 axis = vec3(cos(psi) * cos(phi), cos(psi) * sin(phi), sin(psi));
  ...
  float4 base = _COLOR_BASE_R * vec4(cos(psi) * cos(phi), cos(psi) * sin(phi), sin(psi), 0.0f);
  ...
  v = (float4)(0.99999f * tmp.x, 0.99999f * tmp.y, 0.99999f * tmp.z, v.w);
  return HSLstoRGB(v);
}
