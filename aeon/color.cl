...
_EPI_ float4 rotate_hsls(float4 v, float2 z_z, __constant float* par, float time){
  ...
  float4 axis = (float4)(cos(psi) * cos(phi), cos(psi) * sin(phi), sin(psi), 0.0f);
  ...
  axis = (float4)(cos(psi) * cos(phi), cos(psi) * sin(phi), sin(psi), 0.0f);
  tmp = rotate3D(tmp, axis, th);
  ...
  float4 base = _COLOR_BASE_R * color(cos(psi) * cos(phi), cos(psi) * sin(phi), sin(psi), 0.0f);
