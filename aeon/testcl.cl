...
__kernel void test(read_only image2d_t fb, write_only image2d_t out, __global char4* g_odata, int kernel_dim, int frame_num)
{
    ...
    uint4 seed = (uint4)((uint)(255.0 * ((frame_num + x) % kernel_dim) / kernel_dim), (uint)(255.0 * ((frame_num + y) % kernel_dim) / kernel_dim), 0, 0);
    ...
    uint4 res = (uint4)seed;//(seed.s0, seed.s1, seed.s2, seed.s3);
