...
_EPI_ float4 simple_color(int idx, read_only image2d_t fb, read_only image3d_t aux, float2 z, float4 seed, __constant float* par, float time){
  ...
  return HSVtoRGB((float4)(time * _SEED_COLOR_SPEED(idx) * 0.1f + a + _SEED_COLOR_DHUE(idx), _SEED_COLOR_S(idx), seed.x * _SEED_COLOR_V(idx), 1.0f));
...
_EPI_ float4 tex_color(int idx, read_only image2d_t fb, read_only image3d_t aux, float2 z, float4 seed, __constant float* par, float time){
  ...
  float seed_c = (2.0 * idx + _SEED_TEX_IDX(idx) + 0.5) / _NUM_AUX;
  ...
  return HSVtoRGB((float4)(time * _SEED_COLOR_SPEED(idx) * 0.1f + a + _SEED_COLOR_DHUE(idx) + seed.x, _SEED_COLOR_S(idx) * seed.y, seed.z * _SEED_COLOR_V(idx), seed.w));
}
